...
\text{and} &~~ \xi_i \ge 0.
\end{align*}
Using a radial basis function (RBF) as kernel we performed a grid search over $C$ and $\gamma$ to find those parameters that provide the highest accuracy on the validation set. We used the open source library ``libsvm'' \cite{chang2011libsvm} wrapped by some octave scripts for setting up data sets and performing the grid search.
