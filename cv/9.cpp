...
int num_objs = 0;
float resultMatrix[max_objs][16];

//camera settings
const int width = 640; 
const int height = 480;
const int camangle = 40; // 35
...
void initVideoStream() {
	...
	if (!cap) {
		...
		cap = cvCaptureFromFile("C:\\Users\\Ralf\\TUM\\AR\\1\\MarkerMovie.mpg");
		if (!cap) {
...
//now we have a B/W image of a supposed Marker
			...
			estimateSquarePose( resultMatrix[cur_obj], corners, 0.045 );
			cur_obj++;
			num_objs = cur_obj;
			...
			for (int i = 0; i<4; ++i) {
				for (int j = 0; j<4; ++j) {
					cout << setw(6);
					cout << setprecision(4);
					cout << resultMatrix[cur_obj][4*i+j] << " ";
				}
				cout << "\n";
			}
			cout << "\n";
			float x,y,z;
			x = resultMatrix[cur_obj][3];
			y = resultMatrix[cur_obj][7];
			z = resultMatrix[cur_obj][11];
...
void display() 
{
	...
	for(int i=0; i<max_objs; i++){
		if(resultMatrix[i][15] != 1) break;
		float resultTransposedMatrix[16];
		for (int x=0; x<4; ++x)
		{
			for (int y=0; y<4; ++y)
			{
				resultTransposedMatrix[x*4+y] = resultMatrix[i][y*4+x];
			}
		}

		float dx = resultMatrix[i+1%num_objs][3] - resultMatrix[i][3];
		float dy = resultMatrix[i+1%num_objs][7] - resultMatrix[i][7];
		float dz = resultMatrix[i+1%num_objs][11] - resultMatrix[i][11];

		// vector length
		float l = sqrtf(dx*dx+dy*dy+dz*dz);

		glRotatef(atan(dx/l), 1, 0, 0);
		glRotatef(atan(dy/l), 0, 1, 0);
		glRotatef(atan(dz/l), 0, 0, 1);
